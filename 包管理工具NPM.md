...
# dependencies(生产依赖) 和 devDependencies（开发依赖）
npm install babel -D （放在开发依赖中）
...
![image-20200909183449855](C:%5CUsers%5CAdministrator%5CAppData%5CRoaming%5CTypora%5Ctypora-user-images%5Cimage-20200909183449855.png)
...
0. 修改为官方的地址 (npm config set registry https://registry.npmjs.org/) ==(镜像地址必须是官方的，淘宝镜像是只读的，不能用来提交自己封装的NPM包)==
...
```sh
npm init 初始化，生成package.json包
npm i 包名 -S 或者 --save （保存在生产依赖中）
npm i 包名 -D 或者 --dev （保存在开发依赖中）
npm i  （安装所有package.json中的依赖）
npm i 包名 -g
npm remove 
npm login
npm publish
...
1. 403 Forbidden - Put https:// registry.npm.taobao.org/extract - no perms

这个问题是因为当前设置的是cnpm，登录到的是cnpm，淘宝镜像是只读的，不能让我们发布自己封装的NPM包，因此要切换到官网地址。
...
// 切换回来以后，再进行提交就可以了。
// 还有一个问题值得注意，你提交的NPM包，名字如果被占用也是不能发布成功的
// 起名字时，不能使用大写、空格和下划线；提交时可以提前在npmjs官方搜索一下，是否有重名文件
```
...
```sh
yarn add jquery@1.11.2 （安装指定版本的工具包）
```
